...
$(function () {
  ...
  function parse() {
    ...
    var status = 0;
    var cronStr =  $("#cron").val().split(' ').map(function (item) {
      if (/^\//.test(item)) {
        item = '*' + item;
        status = 1;
      }
      else if(/^\d+\//.test(item)) {
        $("#runTime").html('not support start with number and /');
        status = -1;
      }
      else if(/^\?$/.test(item)) {
        item = '*';
    ...
    console.log('cron val: ', $("#cron").val());
    try {
      var interval = cronParser.parseExpression($("#cron").val());
      var strHTML = '<ul style="margin: 0;">';
      for(var i = 0; i < 5; i++) {
        var date = interval.next().toDate();
        console.log(date);
        strHTML += "<li>" + formatDate('yyyy/MM/dd hh:mm:ss', date) + "</li>";
      }
